...
interface OccasionTableProps {
  ...
  onDeleteOccasion: (id: number) => void;
  onViewOccasion: (id: number) => void;
  onEditOccasion: (id: number) => void;
...
export const OccasionTable: React.FC<OccasionTableProps> = ({
  ...
  const startItem =
    occasions.length === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1;
  const endItem = Math.min(currentPage * itemsPerPage, occasions.length);
  ...
  const handleSelectAll = () => {
    ...
    if (newSelectAll) {
      const currentPageOccasionIds = occasions
        .slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
        .map((occasion) => occasion.id);
      setSelectedOccasions(currentPageOccasionIds);

      if (currentPageOccasionIds.length > 0) {
        onSelectOccasion(currentPageOccasionIds[0]);
      }
    } else {
  ...
  const handleSelectOccasion = (id: number, checked: boolean) => {
    let newSelectedOccasions: number[];

    if (checked) {
      newSelectedOccasions = [...selectedOccasions, id];
    } else {
      newSelectedOccasions = selectedOccasions.filter(
        (occasionId) => occasionId !== id
      );
    }

    setSelectedOccasions(newSelectedOccasions);

    if (newSelectedOccasions.length > 0) {
      onSelectOccasion(newSelectedOccasions[0]);
    } else {
      onSelectOccasion(null);
    }
    ...
    setSelectAll(
      currentPageOccasionIds.every((id) => newSelectedOccasions.includes(id))
    );
  };
  ...
  return (
    <div className="font-gilroyRegular tracking-wider overflow-x-auto">
      {isLoading ? (
        <div className="py-10 text-center">Loading occasions...</div>
      ) : isError ? (
        ...
        <>
                ...
                occasions
                  ...
                  .map((occasion) => (
                    <tr
                      key={occasion.id}
                      className={`border-t ${
                        selectedOccasion === occasion.id ? "bg-blue-50" : ""
                      }`}
                      onClick={() =>
                        onSelectOccasion(
                    ...
                    >
                      <td className="py-3 px-4">
                        <input
                          ...
                          onChange={(e) => {
                            e.stopPropagation();
                            handleSelectOccasion(occasion.id, e.target.checked);
                          }}
                        />
                      </td>
          ...
          <div className="flex justify-between items-center mt-4 text-sm">
            <div className="hidden md:flex">
              Showing {startItem} to {endItem} of {occasions.length} entries
            </div>
