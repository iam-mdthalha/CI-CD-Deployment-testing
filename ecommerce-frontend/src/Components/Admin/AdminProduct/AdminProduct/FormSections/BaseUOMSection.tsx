...
export const BaseUOMSection: React.FC<BaseUOMSectionProps> = ({
  formData,
  setFormData,
  mode,
  open,
  ...
  return (
    <div className="font-gilroyRegular tracking-wider border rounded-md">
      ...
      >
        <h3 className="text-lg font-medium">Base UOM</h3>
        {open ? (
          <svg
      ...
      {open && (
        <div className="px-4 pb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* <div className="flex items-center justify-center gap-2 mt-5">
                        <input type="checkbox" disabled={mode === "view"} className="h-4 w-4" />
                        <label className="text-sm text-gray-700">Apply to all UOM</label>
                    </div> */}
        </div>
      )}
    </div>
