...
export const PatternForm: React.FC<PatternFormProps> = ({
  ...
  const [formData, setFormData] = useState<Pattern>(initialValues);

  const handleChange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>
  ) => {
    const { name, value } = e.target;
    if (name === "prd_pattern_desc" && value.length > 200) return;
      ...
      <div className="font-gilroyRegular tracking-wider bg-white px-6 py-12 rounded-md shadow-md my-6">
        <div className="grid grid-cols-1 gap-4">
          <div>
            ...
            <input
              type="text"
              name="prd_pattern_desc"
              value={formData.prd_pattern_desc}
              onChange={handleChange}
              className="w-full px-3 py-2 border rounded-md text-sm border-gray-300 focus:outline-none focus:ring-0"
              required
              readOnly={mode === "view"}
              maxLength={200}
              placeholder="Enter pattern description (max 200 chars)"
            />
            <div className="text-xs text-gray-500 mt-1 text-right">
              {formData.prd_pattern_desc.length}/200 characters
            </div>
          </div>
        </div>
      </div>
