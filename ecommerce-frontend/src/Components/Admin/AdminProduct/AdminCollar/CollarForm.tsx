...
interface CollarFormProps {
  ...
  onSubmit: (data: Collar) => void;
  onCancel: () => void;
  mode: "add" | "edit" | "view";
}

export const CollarForm: React.FC<CollarFormProps> = ({
  initialValues,
  onSubmit,
  onCancel,
  mode,
}) => {
  const [formData, setFormData] = useState<Collar>(initialValues);

  const handleChange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>
  ) => {
    const { name, value } = e.target;
    if (name === "collar_description" && value.length > 400) return;
        ...
        <div className="grid grid-cols-1 gap-4">
          <div>
            ...
            <input
              ...
              readOnly={mode === "view"}
              maxLength={400}
              placeholder="Enter collar description (max 400 chars)"
            />
            <div className="text-xs text-gray-500 mt-1 text-right">
              {formData.collar_description.length}/400 characters
            </div>
          </div>
        </div>
      </div>
