...
interface SleeveFormProps {
  initialValues: Sleeve;
  onSubmit: (data: Sleeve) => void;
  onCancel: () => void;
  mode: "add" | "edit" | "view";
}

export const SleeveForm: React.FC<SleeveFormProps> = ({
  initialValues,
  onSubmit,
  onCancel,
  mode,
}) => {
  const [formData, setFormData] = useState<Sleeve>(initialValues);

  const handleChange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>
  ) => {
    const { name, value } = e.target;
    if (name === "prd_sleeve" && value.length > 400) return;
        ...
        <div className="grid grid-cols-1 gap-4">
          <div>
            ...
            <input
              type="text"
              name="prd_sleeve"
              value={formData.prd_sleeve}
              onChange={handleChange}
              className="w-full px-3 py-2 border rounded-md text-sm border-gray-300 focus:outline-none focus:ring-0"
              required
              readOnly={mode === "view"}
              maxLength={400}
              placeholder="Enter sleeve description (max 400 chars)"
            />
            <div className="text-xs text-gray-500 mt-1 text-right">
              {formData.prd_sleeve.length}/400 characters
            </div>
          </div>
        </div>
      </div>
