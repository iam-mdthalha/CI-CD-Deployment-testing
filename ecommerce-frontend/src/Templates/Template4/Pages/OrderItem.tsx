...
const OrderItem = ({ orderId, det }: OrderItemProps) => {
  ...
  const isShippingApiEnabled = ecomConfig?.isShippingApiEnabled || false;
  ...
  useEffect(() => {
    const fetchTracking = async () => {
      if (!isShippingApiEnabled || !det.waybillNo) return;
    ...
    fetchTracking();
  }, [det.waybillNo, trackOrderRequest, isShippingApiEnabled]);
  ...
  return (
    <div
      className="border-b border-gray-200 bg-light bg-opacity-50 p-4 sm:p-6"
      data-aos="fade-up"
    >
      {isRefundModalOpen && (
        <div
            ...
            <div className="relative bg-vintageBg rounded-lg shadow-sm border border-gray-200">
              ...
              <div className="p-4 md:p-5 text-center">
                ...
                <button
                  onClick={() => setIsRefundModalOpen(false)}
                  type="button"
                  className="py-2.5 px-5 ms-3 text-sm font-medium text-white focus:outline-none bg-yellow-500 rounded-none border border-gray-200 hover:bg-opacity-90 focus:z-10 focus:ring-4 focus:ring-gray-100 font-gilroyRegular transition-colors"
                >
                  No, cancel
                </button>
              </div>
        ...
        <div className="flex items-start cursor-pointer">
          ...
          <div className="ml-4 flex-1">
              ...
              {!det.refundInitiated && (
                <div className="sm:text-right">
                  <button
                    ...
                    className="text-vintageText hover:text-green-700 text-sm flex gap-1 items-center justify-end font-gilroyRegular transition-colors"
                    style={{ color: "#326638" }}
                  >
                    <svg
                      className="w-4 h-4"
            ...
            {isShippingApiEnabled && (
              <div
                className="mt-3 p-4 rounded-md shadow-sm border font-gilroyRegular text-sm transition-colors"
                style={{
                  backgroundColor: "rgba(244, 223, 180, .3)",
                  borderColor: "#326638",
                }}
              >
                {loadingTracking ? (
