...
const Merchandise: React.FC = () => {
  const { merchandise } = useParams<{ merchandise: string }>();
  ...
  const filterParams = useMemo(() => {
    ...
    return params;
  }, [selectedFilters, activePage, pageSize, merchandise]);

  const hasFilters = Object.values(selectedFilters).some((sel) => sel.length > 0);
  const hasNonPriceFilters = Object.entries(selectedFilters).some(
    ([key, sel]) => key !== "price" && sel.length > 0
  );

  // Query merchandise products list (skip if filters exist or no merchandise param or search active)
  const { data: bookMerchandiseProducts, isLoading: bookMerchandiseLoading } =
    useGetListOfBookMerchandiseProductsQuery(
      {
        merchandiseNames: merchandise ? [merchandise] : [],
        page: activePage,
        productsCount: pageSize,
      },
      {
        skip: Boolean(searchTerm) || hasFilters || !merchandise,
      }
    );

  // Query filtered products only if filters exist, and no search, and merchandise param
  const { data: filteredProducts, isLoading: filteredLoading } =
    useGetListOfBookProductsWithFilterQuery(filterParams, {
      skip: Boolean(searchTerm) || !hasFilters || !merchandise,
    });

  // Query search API if searchTerm exists
  ...
  const merchandiseResponse = bookMerchandiseProducts as ProductResponse;
  const filteredResponse = filteredProducts as ProductResponse;
  const searchResponse = searchResults as ProductResponse;

  const merchandiseProductsData =
    merchandiseResponse?.results?.products || merchandiseResponse?.products || [];
  const merchandiseTotalProducts =
    merchandiseResponse?.results?.totalProducts ||
    merchandiseResponse?.totalProducts ||
    0;

  const filteredProductsData =
    filteredResponse?.results?.products || filteredResponse?.products || [];
  const filteredTotalProducts =
    filteredResponse?.results?.totalProducts || filteredResponse?.totalProducts || 0;
  ...
  const currentProducts = useMemo(() => {
    ...
    if (merchandiseProductsData.length > 0 && !hasFilters) {
      return handleSort(merchandiseProductsData, sort);
    }

    if (
      merchandiseProductsData.length > 0 &&
      hasFilters &&
      !hasNonPriceFilters
    ) {
      let filtered = [...merchandiseProductsData];
  ...
  const isLoading = searchTerm && searchProductsData.length > 0
    ? searchLoading
    : !hasFilters
    ? bookMerchandiseLoading
    : filteredLoading;
  ...
  const totalProducts = useMemo(() => {
    ...
    if (!hasFilters) {
      return merchandiseTotalProducts;
    }
