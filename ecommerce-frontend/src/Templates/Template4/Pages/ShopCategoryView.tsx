...
const Category: React.FC = () => {
  ...
  const { label: resultsLabel, apiParam: categoryParam } = getResultsInfo();
    ...
    useGetListOfBookProductsQuery(
      {
        ...
        subClass: subcategory || undefined,
      },
      {
        skip: hasFilters || !!searchTerm,
      }
    );
  ...
  const bookProductsData =
    bookProductsResponse?.results?.products || bookProductsResponse?.products || [];
  ...
  const currentProducts = useMemo(() => {
    if ((hasFilters || searchTerm) && filteredProductsData.length > 0) {
    ...
    if (bookProductsData.length > 0 && !hasFilters && !searchTerm) {
