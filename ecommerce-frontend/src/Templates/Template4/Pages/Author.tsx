...
const Author: React.FC = () => {
  ...
  const filterParams = useMemo(() => {
    ...
    return params;
  }, [selectedFilters, activePage, pageSize, author]);
  ...
  // Use author's product list query (skip if searching)
  const { data: bookAuthorsProducts, isLoading: bookAuthorsLoading } =
    useGetListOfBookAuthorsProductsQuery(
      {
      ...
      {
        skip: Boolean(searchTerm) || hasFilters || !author,
      }
    );

  // Use filtered products query (skip if searching)
  const { data: filteredProducts, isLoading: filteredLoading } =
    useGetListOfBookProductsWithFilterQuery(filterParams, {
      skip: Boolean(searchTerm) || !hasFilters || !author,
    });

  // Use search API query if searchTerm is present
  const {
    data: searchResults,
  ...
  } = useGetProductsBySearchQuery(
    {
      ...
      searchOffsetValue: "", // Removed author filter here as per fix
      pageSize,
      activePage,
    },
    { skip: !searchTerm }
  ...
  const authorResponse = bookAuthorsProducts as ProductResponse;
  const filteredResponse = filteredProducts as ProductResponse;
  const searchResponse = searchResults as ProductResponse;

  const authorProductsData =
    authorResponse?.results?.products || authorResponse?.products || [];
  const authorTotalProducts =
    authorResponse?.results?.totalProducts ||
    authorResponse?.totalProducts ||
    0;
  ...
  // Decide which products to show based on search input and filters
  const currentProducts = useMemo(() => {
    if (searchTerm && searchProductsData.length > 0) {
      let products = [...searchProductsData];
    ...
    if (authorProductsData.length > 0 && !hasFilters) {
      return handleSort(authorProductsData, sort);
    }

    if (authorProductsData.length > 0 && hasFilters && !hasNonPriceFilters) {
      let filtered = [...authorProductsData];
      ...
      <div className="py-3 md:py-12 px-6 lg:px-24 flex gap-8">
        ...
        <main className="flex-1">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8 text-vintageText">No Products Available</div>
          ) : (
