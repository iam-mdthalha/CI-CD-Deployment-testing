...
const Academics: React.FC = () => {
  const { academic } = useParams<{ academic: string }>();
  ...
  const filterParams = useMemo(() => {
    ...
    return params;
  }, [selectedFilters, activePage, pageSize, academic]);
  ...
  const { data: bookAcademicProducts, isLoading: bookAcademicLoading } =
    useGetListOfBookAcademicProductsQuery(
      {
        academicNames: academic ? [academic] : [],
        page: activePage,
        productsCount: pageSize,
      },
      { skip: Boolean(searchTerm) || hasFilters || !academic }
    );
  ...
  const academicResponse = bookAcademicProducts as ProductResponse;
  const filteredResponse = filteredProducts as ProductResponse;
  const searchResponse = searchResults as ProductResponse;

  const academicProductsData =
    academicResponse?.results?.products || academicResponse?.products || [];
  const academicTotalProducts =
    academicResponse?.results?.totalProducts ||
    academicResponse?.totalProducts ||
    0;
  ...
  const currentProducts = useMemo(() => {
    ...
    if (academicProductsData.length > 0 && !hasFilters) {
      return handleSort(academicProductsData, sort);
    }

    if (academicProductsData.length > 0 && hasFilters && !hasNonPriceFilters) {
      let filtered = [...academicProductsData];
