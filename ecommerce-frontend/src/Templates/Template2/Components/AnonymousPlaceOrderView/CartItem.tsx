...
const CartItem = ({ cartProduct }: Props) => {
  ...
  const quantity = useSelector((state: RootState) => state.cart.cartList).find(
    (item) => item.productId === cartProduct.product.item
  )?.quantity;
  ...
  const incrementQuantity = async () => {
    const availableQuantity = cartProduct.product.stockQty;
    ...
    <Flex direction="row">
      <Stack align="center" gap={0} flex="10%">
        {!cartProduct.imagePath ? (
          ...
          <Image
            ...
            src={cartProduct.imagePath || "placeholder"}
        ...
        <div style={{ paddingBlock: "10px" }}>
          {quantity && (
            <QuantitySelector
              value={quantity}
              productQuantity={cartProduct.product.stockQty}
      ...
      <Paper px={"20px"} flex="85%">
        <Flex direction="column" rowGap={5}>
          <Link
            to={`/${encodeURIComponent(cartProduct.product.item)}`}
      ...
      <ActionIcon
        ...
        onClick={() => {
          dispatch(removeFromCart({ productId: cartProduct.product.item, size }));
