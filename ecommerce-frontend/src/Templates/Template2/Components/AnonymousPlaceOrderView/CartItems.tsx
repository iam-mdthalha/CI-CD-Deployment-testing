...
const CartItem: React.FC<CartItemProps> = ({
  ...
  const cartItem = useSelector((state: RootState) => state.cart.cartList).find(
    (cartItem: any) => cartItem.productId === item.product.item
  );
  ...
  const removeItem = async () => {
    ...
    if (token) {
      try {
        ...
        if (res) {
          dispatch(removeFromCart({ productId: removedProductId, size: removedSize }));
  ...
  const calculateDiscountedPrice = () => {
    ...
    // Use the first active ByValue promotion
