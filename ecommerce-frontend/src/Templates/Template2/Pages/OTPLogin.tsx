...
const OTPLogin: React.FC = () => {
  ...
  const [loginWithPassword, { isLoading: loginWithPasswordLoading }] =
    useLoginWithPasswordMutation();
  const [loginWithOTP, { isLoading: loginWithOTPLoading }] =
    useLoginWithOTPMutation();
  ...
  const {
    ...
    const handleOTPLoginSignIn = async () => {
        const preOTPLoginResult = await preOTPLogin({
            authenticationId: authenticationId,
            anonymous: false
        });

        const otpSentResult = await sendFast2SmsOtp(
            authenticationId
        ).unwrap();
  ...
  const onLoginWithOTPSubmit = async (data: LoginWithOTPSchema) => {
      ...
      try {
        const userData = await loginWithOTP(loginPayload).unwrap();
    ...
    <div className="min-h-screen bg-gray-100 font-montserrat tracking-wider dark:bg-gray-900 flex items-center justify-center px-4">
      {isSubmitting ? (
        ...
        <div className="w-full max-w-md bg-white dark:bg-gray-800 p-8 rounded-xl shadow-md space-y-6">
          ...
          <form
            onSubmit={loginWithOTPHandleSubmit(onLoginWithOTPSubmit)}
            className="space-y-5"
          >
            <div>
              <label
              ...
              />
                {loginWithOTPErrors.otp && (
                  <p className="text-red-500 text-sm">{loginWithOTPErrors.otp.message}</p>
                )}
              {otpSent && (
        ...
        <div className="w-full max-w-md bg-white dark:bg-gray-800 p-8 rounded-xl shadow-md space-y-6">
          ...
          >
            ...
            <CustomDarkButtonFull type="button" disabled={isSubmitting} onClick={handleOTPLoginSignIn}>
              {isSubmitting ? (
                <svg
                  className="animate-spin h-5 w-5 text-white mx-auto"
                  xmlns="http://www.w3.org/2000/svg"
