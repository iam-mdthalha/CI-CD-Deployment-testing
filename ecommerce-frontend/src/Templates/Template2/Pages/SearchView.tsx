...
import { useGetListOfProductsbygroupQuery } from "Services/ProductApiSlice";
...
const SearchView = () => {
  ...
  const searchOffsetValue = searchParams.get("offset") || "all";
  ...
  const queryParams = {
    ...
    ...(filterCategories.brand && { brand: filterCategories.brand }),
    ...(filterCategories.department && {
      department: filterCategories.department,
    }),
    ...(filterCategories.subCategory && {
      subCategory: filterCategories.subCategory,
    }),
    ...(filterCategories.fromprice &&
      filterCategories.Toprice && {
        items: [
          `price:${filterCategories.fromprice}-${filterCategories.Toprice}`,
        ],
      }),
  };

  const { data: productsData, isLoading } = useGetListOfProductsbygroupQuery(
    queryParams,
    {
      skip: !searchValue.trim() && categoryValue === "all",
    }
  );
  ...
  const currentProducts = useMemo(() => {
    ...
    if (filterCategories.fromprice && filterCategories.Toprice) {
      ...
      filtered = filtered.filter(
        (product) =>
          product.product.ecomUnitPrice >= minPrice &&
          product.product.ecomUnitPrice <= maxPrice
      );
    }

    return handleSort(filtered, sort);
  }, [productsData, filterCategories, sort]);
  ...
  const paginate = (pageNumber: number) => {
    ...
    if (searchOffsetValue !== "all") {
      params.offset = searchOffsetValue;
    }
  ...
  useEffect(() => {
    setActivePage(1);
  }, [selectedFilters, categoryValue, searchValue]);
  ...
  return (
    <div className="min-h-screen font-montserrat tracking-widest container mx-auto px-4 py-8 bg-gray-50">
