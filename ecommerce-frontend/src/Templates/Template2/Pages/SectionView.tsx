...
import { useGetListOfProductsWithFilterbygroupQuery } from "Services/ProductApiSlice";
...
import { ProductFilterParams } from "Types/ProductPackerDTO";
...
const SectionView = () => {
  ...
  const currentProducts = useMemo(() => {
    ...
    return sectionDetail?.sectionProducts?.products || [];
  }, [
    selectedFilters,
    filteredProducts,
  ...
  const sortedProducts = useMemo(() => {
    return handleSort([...currentProducts], sort);
  ...
  const totalProducts = useMemo(() => {
    ...
    return sectionDetail?.sectionProducts?.totalProducts || 0;
  }, [
    selectedFilters,
    filteredProducts?.totalProducts,
    sectionDetail?.sectionProducts?.totalProducts,
  ]);

  const isLoading =
    sectionDetailLoading || (selectedFilters.length > 0 && filteredLoading);
